...
export default function AdminTruchita() {
  ...
  const addProduct = (section: MenuSection) => {
    ...
    const defaults: Record<MenuSection, Pick<Product, 'name' | 'description' | 'image' | 'hasIngredients' | 'service'>> = {
      ...
      bolsa: { name: 'UN GUSTITO MÁS', description: 'Botana abierta y preparada con el esquite elegido.', image: '/botanas-en-bolsa.png', hasIngredients: true, service: 'bag' },
      'muy-mexicano': { name: 'NUEVA OPCIÓN AL CARBÓN', description: 'Preparación 100% al carbón.', image: '/hero-corn.png', hasIngredients: false, service: 'cup' },
    };
    setDraft((current) => ({ ...current, products: [...current.products, { id, section, price: 0, available: true, ...defaults[section] }] }));
  };

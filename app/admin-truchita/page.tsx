...
export default function AdminTruchita() {
  ...
  const traditional = useMemo(
    () => draft.products.filter((product) => product.section === "traditional"),
    [draft.products],
  );
  const specialties = useMemo(
    () => draft.products.filter((product) => product.section === "specialty"),
    [draft.products],
  );
  const elotes = useMemo(
    () => draft.products.filter((product) => product.section === "elotes"),
    [draft.products],
  );
  const bags = useMemo(
    () => draft.products.filter((product) => product.section === "bolsa"),
    [draft.products],
  );
  ...
  const updateExtra = (id: string, changes: Partial<ExtraOption>) => {
    setDraft((current) => ({
      ...
      extras: current.extras.map((extra) =>
        extra.id === id ? { ...extra, ...changes } : extra,
      ),
    }));
  };
  ...
  const addProduct = (section: MenuSection) => {
    ...
    const defaults: Record<
      ...
      Pick<
        Product,
        "name" | "description" | "image" | "hasIngredients" | "service"
      >
    > = {
      traditional: {
        name: "NUEVO ESQUITE",
        description: "Describe aquí lo que lleva.",
        image: "/esquites-charola-carbon.png",
      ...
      elotes: {
        name: "NUEVO ELOTE",
        description: "Elote entero preparado al carbón.",
        image: "/hero-corn.png",
        hasIngredients: false,
        service: "corn",
      },
      bolsa: {
        name: "UN GUSTITO MÁS",
        description: "Botana abierta y preparada con el esquite elegido.",
        image: "/botanas-en-bolsa.png",
        hasIngredients: true,
        service: "bag",
      },
      "muy-mexicano": {
        name: "NUEVA OPCIÓN AL CARBÓN",
        description: "Preparación 100% al carbón.",
        image: "/esquites-charola-carbon.png",
        hasIngredients: false,
        service: "cup",
      },
    ...
    setDraft((current) => ({
      ...
      products: [
        ...current.products,
        { id, section, price: 0, available: true, ...defaults[section] },
      ],
    }));
  };
    ...
    setDraft((current) => ({
      ...
      extras: [
        ...current.extras,
        {
          ...
          name: "NUEVO EXTRA",
          price: 0,
          description: "Describe este extra.",
          imagePosition: "elote",
        },
      ],
    }));
  ...
  const save = async () => {
    ...
    setMessage(
      response.ok
        ? "Listo: el menú público ya fue actualizado."
        : payload && typeof payload === "object" && "message" in payload
          ? String(payload.message)
      ...
      <main className="admin-shell">
        <div className="admin-wrap admin-login">
          ...
          <div className="admin-card">
            <p className="modal-kicker">ACCESO PRIVADO</p>
            <h2>ADMINISTRACIÓN</h2>
            <p className="checkout-note">
              Este panel no aparece en la página pública.
            </p>
    ...
    <main className="admin-shell">
      <div className="admin-wrap">
        <header className="admin-brand">
          ...
          <p>PANEL PRIVADO · MENÚ Y PRODUCTOS</p>
        </header>
        <div className="admin-intro">
          <div>
            <p className="modal-kicker">TODO LO QUE VE TU CLIENTE</p>
            <h1>
              CONTROL
              <br />
              DE LA BRASA.
          ...
          <p>
            Edita nombres, precios, ingredientes e imágenes. Nada de esto se
            muestra como enlace en la página pública.
          </p>
        </div>
        <div className="admin-toolbar">
          <p>{message || "Haz tus cambios y publícalos cuando estés lista."}</p>
          <div>
            <a
        ...
        <section className="admin-section">
          ...
          <div className="admin-extras">
            {draft.extras.map((extra) => (
              <article className="admin-extra" key={extra.id}>
                <label className="admin-field">
                  <span>NOMBRE</span>
                  <input
                    value={extra.name}
                    onChange={(event) =>
                      updateExtra(extra.id, { name: event.target.value })
                    }
                  />
                </label>
                <label className="admin-field">
                ...
                <label className="admin-checkbox">
                  <input
                    type="checkbox"
                    checked={Boolean(extra.onlyWithIngredients)}
                    onChange={(event) =>
                      updateExtra(extra.id, {
                        onlyWithIngredients: event.target.checked,
                      })
                    }
                  />
                  SOLO PARA CHAROLAS CON INGREDIENTES
                </label>
                <label className="admin-field">
                  <span>DISPONIBLE PARA</span>
                  <select
                    value={extra.onlyForServices?.[0] ?? "all"}
                    onChange={(event) =>
                      updateExtra(extra.id, {
                        onlyForServices:
                          event.target.value === "all"
                            ? undefined
                            : [event.target.value as "cup" | "corn" | "bag"],
                      })
                    }
                  >
                    <option value="all">Todos los formatos</option>
                    <option value="cup">Charolas de esquites</option>
                    <option value="corn">Elotes enteros</option>
                    <option value="bag">Un gustito más</option>
                  </select>
                </label>
                <button
                  className="admin-button ghost"
...
function AdminProductSection({
  ...
  products: Product[];
  section: MenuSection;
  onAdd: (section: MenuSection) => void;
  onRemove: (id: string) => void;
  onUpdate: (id: string, changes: Partial<Product>) => void;
  onUpload: (id: string, event: ChangeEvent<HTMLInputElement>) => void;
}) {
  return (
    <section className="admin-section">
      <div className="admin-section-head">
          ...
          >
            ...
            <div className="admin-product-grid">
              ...
              <label className="admin-field wide">
                <span>DETALLE / INGREDIENTES</span>
                <textarea
                  value={product.description}
                  onChange={(event) =>
                    onUpdate(product.id, { description: event.target.value })
                  }
                />
              </label>
              <label className="admin-field">
              ...
              <label className="admin-field">
                <span>FORMATO</span>
                <select
                  value={product.service ?? "cup"}
                  onChange={(event) =>
                    onUpdate(product.id, {
                      service: event.target.value as Product["service"],
                    })
                  }
                >
                  <option value="cup">Charola de esquites</option>
                  <option value="corn">Elote entero</option>
                  <option value="bag">Esquite en bolsa</option>
                </select>
              </label>
              <label className="admin-checkbox">
                <input
                  type="checkbox"
                  checked={product.hasIngredients}
                  onChange={(event) =>
                    onUpdate(product.id, {
                      hasIngredients: event.target.checked,
                    })
                  }
                />
                LLEVA INGREDIENTES
              </label>
              <label className="admin-checkbox availability-toggle">
                <input
                  type="checkbox"
                  checked={product.available !== false}
                  onChange={(event) =>
                    onUpdate(product.id, { available: event.target.checked })
                  }
                />
                DISPONIBLE HOY
              </label>
              <label className="admin-field wide">
                ...
                <div className="admin-image-row">
                  <input
                    value={product.image}
                    onChange={(event) =>
                      onUpdate(product.id, { image: event.target.value })
                    }
                  />
                  <label
                  ...
                  >
                    <input
                      accept="image/*"
                      type="file"
                      onChange={(event) => onUpload(product.id, event)}
                    />
                  </label>
                </div>
              </label>
              <figure className="admin-image-preview wide">
                <img
                  src={product.image || "/esquites-charola-carbon.png"}

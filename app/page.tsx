...
function randomWelcome() {
  const last = window.sessionStorage.getItem("truchita-welcome-phrase");
  const options = welcomeMessages.filter((message) => message !== last);
  const message =
    options[Math.floor(Math.random() * options.length)] ?? welcomeMessages[0];
  window.sessionStorage.setItem("truchita-welcome-phrase", message);
  return message;
}
...
export default function Home() {
  ...
  const [maicitoMessage, setMaicitoMessage] = useState(welcomeMessages[0]);
  ...
  useEffect(() => {
    ...
    setIsSaturday(weekday === "Sat");
    setMaicitoMessage(randomWelcome());
  }, []);
  ...
  return (
    <main className="landing-page">
      <header className="site-header landing-header">
        ...
        <a className="header-order header-menu" href="/menu" onClick={openMenu}>
          VER MENÚ <i aria-hidden="true">↗</i>
        </a>
      </header>
      ...
      >
        ...
        <div className="vintage-burn burn-one" aria-hidden="true" />
        ...
        <div className="hero-content">
          <p className="eyebrow">
            <span />
            ZACAPOAXTLA, PUEBLA <span />
          </p>
          <h1 id="hero-title">
            <span className="esquites">ESQUITES</span>
            <span className="truchita">EL TRUCHITA</span>
            <span className="carbon">AL CARBÓN</span>
          </h1>
          <p className="hero-sentence">
            Honramos el maíz, el fuego y la tradición que nos une.
            <br />
            Sabor mexicano, hecho especialmente para ti.
          </p>
        ...
        <aside className="future-space" id="domicilio">
          ...
          <span>
            Estamos preparando un espacio para que nuestros clientes degusten su
            preparación tranquilamente.
          </span>
        </aside>
        <div className="hero-edge">
          <span>SOLO A DOMICILIO</span>
          <b>✦</b>
          <span>DOM–VIE · 6:30 P. M. — 12:30 A. M.</span>
          <b>✦</b>
      ...
      {openingMenu && (
        <div
          className="arcade-menu-transition"
          aria-live="polite"
          aria-label="Cargando menú"
        >
          <div className="arcade-scanlines" aria-hidden="true" />
          <div className="arcade-corn-field" aria-hidden="true">
            {Array.from({ length: 28 }, (_, index) => (
              <i
                key={index}
                style={{ "--kernel-index": index } as CSSProperties}
              >
                ●
              </i>
            ))}
          </div>
          <div className="arcade-transition-copy">
            <p>EL TRUCHITA · INSERTAR ANTOJO</p>
            <strong>
              CARGANDO
              <br />
              <em>MENÚ</em>
            </strong>
            <span>MAÍZ AL CARBÓN · 100 PTS</span>
            <b>••••••••••</b>
          </div>
        </div>
      )}

...
type SpiceOption = { id: string; name: string; note: string; color: string };
...
const snackFlavors: SnackFlavor[] = [
  ...
  },
  {
    ...
    note: "Para quien quiere picante.",
    color: "flamin",
    price: 30,
  },
  {
  ...
  },
  {
    id: "tostitos",
    name: "TOSTITOS",
    note: "El clásico para llenar bien.",
    color: "toast",
    price: 35,
  },
];
const spiceOptions: SpiceOption[] = [
  {
    id: "sin-picante",
    name: "SIN PICANTE",
    note: "Solo el sabor de la brasa.",
    color: "calm",
  },
  {
    id: "pepita-chiltepin",
    name: "PEPITA + CHILTEPÍN",
    note: "Tostadito, aromático y bravo.",
    color: "pepita",
  },
  {
    id: "cacahuate-ajonjoli",
    name: "CACAHUATE + AJONJOLÍ + CHILTEPÍN",
    note: "Con chiltepín para cerrar fuerte.",
    color: "cacahuate",
  },
  {
    id: "cheetos-flamin",
    name: "CHEETOS FLAMIN’ HOT",
    note: "Crujiente y picosito.",
    color: "flamin",
  },
  {
    id: "doritos-fuego",
    name: "DORITOS FUEGO",
    note: "Chile, limón y antojo.",
    color: "fuego",
  },
];
...
const panelDetails: Array<{
...
}> = [
  {
    key: "traditional",
    nav: "TRADICIONALES",
    eyebrow: "LOS DE SIEMPRE. PERO AQUÍ EMPIEZAN EN EL CARBÓN.",
    title: "LOS TRADICIONALES",
    level: "NIVEL 01",
    icon: "◈",
    clue: "Lo clásico, pero con brasas.",
  },
  {
    key: "specialty",
    nav: "ESPECIALES",
    eyebrow: "SABORES DE MÉXICO LLEVADOS AL ESQUITE.",
    title: "ESPECIALES DE LA CASA",
    level: "NIVEL 02",
    icon: "✹",
    clue: "Para entrarle con hambre.",
    accent: true,
  },
  {
    key: "elotes",
    nav: "ELOTES",
    eyebrow: "DEL ASADOR A TUS MANOS.",
    title: "ELOTES AL CARBÓN",
    level: "NIVEL 03",
    icon: "♨",
    clue: "Enteros, tostados y sin rodeos.",
  },
  {
    key: "bolsa",
    nav: "UN GUSTITO MÁS",
    eyebrow: "ABRIMOS LA BOTANA. EL RESTO LO ARMAS A TU GUSTO.",
    title: "UN GUSTITO MÁS",
    level: "NIVEL 04",
    icon: "▰",
    clue: "Botana abierta, carbón adentro.",
    accent: true,
  },
  {
    key: "muy-mexicano",
    nav: "MUY MEXICANO",
    eyebrow: "MAÍZ, BRASA Y OFICIO. NADA MÁS.",
    title: "MUY MEXICANO",
    level: "NIVEL 05",
    icon: "✦",
    clue: "Maíz, fuego y oficio.",
  },
];
...
function IngredientReference({
  ...
  kind: "mayo" | "queso" | "elote" | "ingredientes";
}) {
  return (
    <span
      className={"ingredient-reference ingredient-" + kind}
      aria-hidden="true"
    />
  );
}
...
function productImageForDisplay(product: Product) {
  if (
    product.service === "cup" &&
    (/esquite-callejero|tradicional-/.test(product.image) || !product.image)
  )
    return "/esquites-charola-carbon.png";
  return product.image || "/esquites-charola-carbon.png";
}

function selectionBasePrice(
  product: Product,
  snackFlavor?: SnackFlavor,
  bagFilling?: Product,
) {
  if (product.service === "bag")
    return (snackFlavor?.price ?? 0) + (bagFilling?.price ?? 0);
  return product.price;
}

function cartItemTotal(item: CartItem) {
  return (
    selectionBasePrice(item.product, item.snackFlavor, item.bagFilling) +
...
function businessStatus(now = new Date()): BusinessStatus {
  ...
  return {
    ...
    isSaturday: day === "Sat",
    label: open
      ? "ABIERTO AHORA · TOMAMOS PEDIDOS"
      : "CERRADO AHORA · DOM–VIE 6:30 P. M. — 12:30 A. M.",
  };
}
...
function deliveryPolicy(total: number): DeliveryPolicy {
  if (total >= 400)
    return {
      title: "ENVÍO GRATIS",
      detail:
        "Tu pedido alcanza el envío gratis. Solo confirmaremos que la dirección esté dentro de nuestra zona de reparto.",
      whatsapp:
        "Envío gratis por subtotal de " +
        money(total) +
        ". Zona de entrega por confirmar.",
    };
  if (total >= 250 && total <= 300)
    return {
      title: "CUBRIMOS LA MITAD DEL ENVÍO",
      detail:
        "Nosotros cubrimos la mitad. El monto final depende de la distancia de entrega.",
      whatsapp:
        "El negocio cubre la mitad del envío. Monto final según distancia.",
    };
  if (total < 250)
    return {
      title: "ENVÍO POR CUENTA DEL CLIENTE",
      detail: "El costo se confirma según la distancia de entrega.",
      whatsapp:
        "Costo de envío por cuenta del cliente; monto por confirmar según distancia.",
    };
  return {
    title: "ENVÍO POR CONFIRMAR",
    detail:
      "En pedidos de $301 a $399 confirmamos el costo según la distancia antes de preparar.",
    whatsapp: "Costo de envío por confirmar según distancia antes de preparar.",
  };
}
...
function nextMaicitoMessage(key: string, messages: string[]) {
  ...
  const message =
    options[Math.floor(Math.random() * options.length)] ?? messages[0];
...
export default function MenuExperience() {
  ...
  useEffect(() => {
    const syncCompanion = () => {
      ...
      setShowCompanion(
        !contact ||
          contact.getBoundingClientRect().top > window.innerHeight * 0.62,
      );
    };
    syncCompanion();
    ...
    () =>
      Object.fromEntries(
        panelDetails.map((panel) => [
          panel.key,
          availableProducts.filter((product) => product.section === panel.key),
        ]),
      ) as Record<MenuSectionType, Product[]>,
    [availableProducts],
  );
  const bagFillings = useMemo(
    () => [...productsBySection.traditional, ...productsBySection.specialty],
    [productsBySection],
  );
  const availableExtras = useMemo(
    () =>
      menu.extras.filter((option) => {
        if (option.onlyWithIngredients && !activeProduct?.hasIngredients)
          return false;
        if (
          option.onlyForServices &&
          !option.onlyForServices.includes(activeProduct?.service ?? "cup")
        )
          return false;
        return true;
      }),
  ...
  const activeBasePrice = useMemo(
    () =>
      activeProduct
        ? selectionBasePrice(activeProduct, snackFlavor, bagFilling)
        : 0,
    [activeProduct, bagFilling, snackFlavor],
  );
  const activePrice = useMemo(
    () =>
      activeBasePrice +
      selectedExtras.reduce((sum, option) => sum + option.price, 0),
    [activeBasePrice, selectedExtras],
  );
  const total = useMemo(
  ...
  const triggerMaicito = (action: Exclude<MaicitoAction, "idle">) => {
    ...
    maicitoTimer.current = window.setTimeout(
      () => setMaicitoAction("idle"),
      action === "celebrate" ? 2100 : 1600,
    );
  };
      ...
      const notes =
        effect === "add"
          ...
          : effect === "open"
            ? [220, 330]
            : [294, 370];
      oscillator.type = effect === "add" ? "square" : "triangle";
  ...
  const whatsappUrl = () => {
    const order = cart
      .map((item, index) => {
        const details = [
          item.preparation === "muy-mexicano"
            ? "Estilo Muy Mexicano: 100% al carbón, sin mantequilla ni especias"
            : "",
  ...
  const currentIndex =
    ...
    " / " +
    String(panelDetails.length).padStart(2, "0");
  ...
  return (
    <main className="menu-page">
      ...
      <aside
        className={
          "maicito-companion " +
          (showCompanion && modal === "none" ? "is-visible" : "")
        }
        aria-label="Acceso rápido a mi pedido"
      >
        <div
          className={"maicito-roamer is-" + maicitoAction}
          aria-hidden="true"
        >
          <img
            src={
              maicitoAction === "guide"
                ? "/maicito-truchita-guide.png"
      ...
      >
        <div className="menu-lead">
          ...
          <div className="menu-lead-side">
            <figure className="menu-photo">
              <img
                src="/esquites-charola-carbon.png"
                alt="Charola de aluminio con esquites asados al carbón, queso, mayonesa y limón"
              />
              <figcaption>
                CHAROLA ALUMINIO · HECHA AL MOMENTO · SOLO A DOMICILIO
              </figcaption>
            </figure>
            <p>
              Elige un nivel. Cada preparación se arma al carbón y sale en
              charola de aluminio.
            </p>
          </div>
        </div>
        ...
        >
          <div className="arcade-level-heading">
            <p>INSERTA TU ANTOJO</p>
            <h2 id="level-select-title">
              ELIGE UN
              <br />
              <em>NIVEL.</em>
            </h2>
            <span>05 NIVELES · HECHOS AL CARBÓN</span>
          </div>
          <div
            className="level-card-grid"
            role="tablist"
            aria-label="Niveles del menú"
          >
            {panelDetails.map((panel, index) => (
              <button
                ...
                className={
                  ...
                  (visitedPanels.includes(index) ? " visited" : "")
                }
                type="button"
                onClick={() => navigatePanel(index)}
                role="tab"
                aria-selected={activePanel === index}
              >
                <span className="level-card-number">{panel.level}</span>
                <i aria-hidden="true">{panel.icon}</i>
                <strong>{panel.nav}</strong>
                <small>{panel.clue}</small>
                <b>{visitedPanels.includes(index) ? "VISTO" : "NUEVO"}</b>
              </button>
            ))}
          </div>
        </section>
        ...
        <div
          className="menu-category-nav arcade-mini-map"
        ...
        >
          <div className="menu-tabs" role="tablist">
            {panelDetails.map((panel, index) => (
              <button
                ...
                className={
                  (activePanel === index ? "active " : "") +
                  (visitedPanels.includes(index) ? "visited" : "unvisited")
                }
                type="button"
                onClick={() => navigatePanel(index)}
                role="tab"
          ...
          <div className="menu-nav-arrows">
            <button
              ...
              onClick={() =>
                navigatePanel(
                  (activePanel + panelDetails.length - 1) % panelDetails.length,
                )
              }
            >
              ←
            ...
            <button
              ...
              onClick={() =>
                navigatePanel((activePanel + 1) % panelDetails.length)
              }
            >
              →
            </button>
            ...
            >
              {panel.key === "bolsa" ? (
                <BagSection
                  ...
                  fillings={bagFillings}
                  onChoose={openCustomizer}
                />
              ) : panel.key === "muy-mexicano" ? (
                <MuyMexicanoSection
                  productsBySection={productsBySection}
                  bagFillings={bagFillings}
                  onChoose={openCustomizer}
                />
              ) : (
                <MenuSection
                  ...
                  title={panel.title}
                  products={productsBySection[panel.key]}
                  onChoose={openCustomizer}
                  accent={panel.accent}
                />
              )}
        ...
        <div className="menu-bottom-line">
          <span>¿YA SABES QUÉ SE TE ANTOJA?</span>
      ...
      <section className="contact" id="contacto">
        <div>
          <p className="section-kicker">CUANDO EL ANTOJO PEGA</p>
          <h2>
            SOLO A<br />
            <em>DOMICILIO.</em>
          </h2>
      ...
      <footer>
        ...
        <p>SOLO A DOMICILIO · DOM–VIE 6:30 P. M. — 12:30 A. M.</p>
        <button
        ...
        >
          {soundOn ? "♪ SONIDOS ON" : "♪ SONIDOS OFF"}
        </button>
        <button type="button" onClick={openCart}>
          MI PEDIDO{" "}
          <i className={cart.length ? "cart-dot active" : "cart-dot"} />
          ...
          >
            ...
            {modal === "sending" && (
              <div className="sending-state">
                <div className="corn-flight" aria-hidden="true">
                  <img src="/maicito-truchita-free.png" alt="" />
                  <i>✦</i>
                  <i>✦</i>
                  <i>✦</i>
                </div>
                <p className="modal-kicker">PREPARANDO TU MENSAJE</p>
                <h2>
                  ¡VA VOLANDO
                  <br />A WHATSAPP!
                </h2>
...
function Customizer({
  ...
  bagFilling?: Product;
  setBagFilling: (value: Product) => void;
  bagFillings: Product[];
    ...
    <>
      <p className="modal-kicker">
        {carbonOnly
          ...
          : serviceLabel(activeProduct)}
      </p>
      <h2>{activeProduct.name}</h2>
      <aside className="exact-ingredients">
        <span>ESTO ES LO QUE LLEVA</span>
        <p>{activeProduct.description}</p>
        {activeProduct.service === "corn" && (
          <small>
            Si agregas ingredientes o más granos, los servimos aparte en una
            charola de aluminio.
          </small>
        )}
      </aside>
      <div className="customizer-arcade-top">
        <figure className={previewClass}>
          <img
            src={previewImage}
            alt={
              activeProduct.service === "bag"
                ? "Botana para personalizar"
                : "Charola de aluminio con esquites al carbón"
            }
          />
        ...
        <div className="arcade-score">
          ...
          <b>{String(selectedCount).padStart(2, "0")} POWER-UPS</b>
        </div>
      </div>
      <ol className="customizer-steps" aria-label="Pasos para personalizar">
        <li className="active">
          <span>01</span> BASE
        </li>
        <li className={selectedCount ? "active" : ""}>
          <span>02</span> TOQUES
        </li>
      ...
      {activeProduct.service === "bag" && (
        <p className="bag-total-note">
          Incluye la botana elegida; sus precios no se muestran por separado.
        </p>
      )}
      {carbonOnly && (
        <p className="carbon-only-note">
          Esta base sale directo de las brasas; los toques los eliges tú.
        </p>
      )}
      <div className="custom-options">
        {activeProduct.service === "bag" && (
          <>
            <fieldset className="bag-customizer">
              <legend>PASO 01 · ¿QUIERES CAMBIAR LA BOTANA?</legend>
              <div className="bag-customizer-options">
                {snackFlavors.map((flavor) => (
                  <label
                    className={snackFlavor.id === flavor.id ? "selected" : ""}
                    key={flavor.id}
                  >
                    <input
                      type="radio"
                      name="snack"
                      checked={snackFlavor.id === flavor.id}
                      onChange={() => setSnackFlavor(flavor)}
                    />
                    <span className={"snack-swatch snack-" + flavor.color} />
                    <b>{flavor.name}</b>
                  </label>
                ))}
              </div>
            </fieldset>
            <fieldset className="bag-filling-customizer">
              <legend>¿QUÉ ESQUITE QUIERES DENTRO?</legend>
              <div>
                {bagFillings.map((filling) => (
                  <label
                    className={bagFilling?.id === filling.id ? "selected" : ""}
                    key={filling.id}
                  >
                    <input
                      type="radio"
                      name="bag-filling"
                      checked={bagFilling?.id === filling.id}
                      onChange={() => setBagFilling(filling)}
                    />
                    <span>
                      {filling.section === "traditional"
                        ? "CLÁSICO"
                        : "ESPECIAL"}
                    </span>
                    <b>{filling.name}</b>
                    <em>{money(filling.price)}</em>
                  </label>
                ))}
              </div>
            </fieldset>
        ...
        <div className="customizer-step-label">
          <span>02</span>
          <b>ELIGE TUS TOQUES</b>
          <small>La charola se arma a tu gusto.</small>
        </div>
        <ToggleRow
          kind="mayo"
          title="MAYONESA"
          description={
            carbonOnly
              ? "Opcional: se agrega aparte si la quieres."
              : "Como te gusta, o sin ella."
          }
          checked={mayo}
          disabled={false}
          onChange={setMayo}
        ...
        <ToggleRow
          ...
          description={
            carbonOnly
              ? "Opcional: se agrega aparte si lo quieres."
              : "Queso para cerrar bien la preparación."
          }
          checked={queso}
          disabled={false}
          onChange={setQueso}
        ...
        <fieldset>
          <legend>
            {activeProduct.service === "corn"
              ? "SI EL ELOTE NO TE BASTA, AGRÉGALE…"
              : "ELIGE TODOS LOS EXTRAS QUE SE TE ANTOJEN"}
          </legend>
          {activeProduct.service === "corn" && (
            <p className="extra-help">
              Si agregas tocino, salchicha, quesos fundidos o más granos, va
              servido en una charola de aluminio aparte — $25 c/u.
            </p>
          )}
          <div className="extra-grid">
            {availableExtras.map((option) => (
              ...
              >
                ...
                />
                <IngredientReference kind={option.imagePosition} />
                <span>
                  <b>{option.name}</b>
        ...
        <label className="note-field">
          <span>
            <b>03</b> NOTA PARA TU PEDIDO
          </span>
          <textarea
            maxLength={180}
            placeholder="Ej. bien picoso, sin limón..."
            value={note}
...
function ToggleRow({
  ...
  return (
    <label
      className={
        disabled
          ? "switch-row switch-row-visual disabled"
          : "switch-row switch-row-visual"
      }
    >
      <span className="switch-option-copy">
        <IngredientReference kind={kind} />
        <span>
          <b>{title}</b>
...
function Maicito({
  ...
  return (
    <figure className={"maicito maicito-" + mood} aria-hidden="true">
      <img
        src={
          mood === "sad"
...
function CartView({
  ...
  const delivery = deliveryPolicy(total);
  const craving = Math.min(
    100,
    15 + cart.length * 25 + Math.floor(total / 40) * 5,
  );
  return (
    <>
      <p className="modal-kicker">ESTO ES LO QUE SE VA A LA BRASA</p>
      <h2>
        MI PEDIDO <span className="cart-count">{cart.length}</span>
      </h2>
      {cart.length === 0 ? (
        ...
        <>
                ...
                <div>
                  ...
                  <p>
                    {item.preparation === "muy-mexicano"
                      ? "100% al carbón · "
                      : ""}
                    {item.snackFlavor ? item.snackFlavor.name + " · " : ""}
                    {item.bagFilling
                      ? "Con " + item.bagFilling.name + " · "
                      : ""}
          ...
          <div className="arcade-receipt">
            <span>RECIBO DE MAQUINITA · BRASA-01</span>
            <b>◼ ◼ ◼</b>
          </div>
          ...
          <div className="craving-meter">
            <span>MEDIDOR DEL ANTOJO</span>
            <div aria-label={"Antojo al " + craving + "%"}>
              <i style={{ width: craving + "%" }} />
            </div>
            <b>{craving}%</b>
          </div>
          ...
          {addedMessage && (
            <aside className="cart-added-message" role="status">
              <Maicito />
              <div>
                <p>{addedMessage}</p>
                <button type="button" onClick={onExplore}>
                  NIVEL SIGUIENTE · {suggestedPanel.nav} ↗
                </button>
              </div>
            </aside>
          )}
          {!isOpen && (
            <aside className="closed-notice">
              <Maicito mood={isSaturday ? "sad" : "sleepy"} />
              <div>
          ...
          <div className="cart-actions">
            ...
            <button type="button" className="empty-button" onClick={onEmpty}>
              VACIAR SELECCIÓN
            </button>
          </div>
          <button
...
function CheckoutForm({
  ...
  const shareExactLocation = () => {
    if (!navigator.geolocation) {
      setLocationStatus(
        "Tu navegador no permite compartir ubicación. Puedes pegar un enlace de Google Maps.",
      );
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () =>
        setLocationStatus(
          "No pudimos obtenerla. Revisa el permiso o pega un enlace de Google Maps.",
        ),
  ...
  return (
    <form onSubmit={onSubmit}>
      <p className="modal-kicker">SERVICIO ÚNICAMENTE A DOMICILIO</p>
      <h2>¿A NOMBRE DE QUIÉN?</h2>
      <p className="checkout-note">
        Folio {reference}. Estos datos van incluidos en tu mensaje de WhatsApp.
      </p>
      <aside className="arcade-receipt checkout-receipt">
        <span>RECIBO FINAL · LISTO PARA CONFIRMAR</span>
        <strong>{money(total)}</strong>
        <b>▣ {reference}</b>
      </aside>
      {!isOpen && (
      ...
      <div className="customer-form">
        ...
        <label>
          ...
          <input
            ...
            pattern="[0-9]{10}"
            maxLength={10}
            title="Escribe un número de 10 dígitos"
        ...
        <label className="form-wide">
          <span>
            REFERENCIAS <small>OPCIONAL</small>
          </span>
          <textarea
            value={customer.references}
            onChange={(event) =>
              setCustomer({ ...customer, references: event.target.value })
            }
            placeholder="Color de portón, entre calles o cualquier referencia."
          />
        </label>
        <div className="form-wide location-field">
          <span>
            UBICACIÓN EXACTA{" "}
            <small>OPCIONAL, MUY ÚTIL PARA EL REPARTIDOR</small>
          </span>
          <div>
            <input
              type="url"
              ...
              onChange={(event) =>
                setCustomer({ ...customer, exactLocation: event.target.value })
              }
              placeholder="Pega un enlace de Google Maps"
            />
            <button type="button" onClick={shareExactLocation}>
              USAR MI UBICACIÓN ACTUAL <b>⌖</b>
            </button>
          </div>
          ...
          <div>
            <label className={customer.payment === "exact" ? "selected" : ""}>
              <input
                type="radio"
                name="payment"
                checked={customer.payment === "exact"}
                onChange={() =>
                  setCustomer({ ...customer, payment: "exact", changeFor: "" })
                }
              />
              <span>
                <b>NO, LLEVO PAGO EXACTO</b>
                <small>Así la entrega es más ágil.</small>
              </span>
            </label>
            <label className={customer.payment === "change" ? "selected" : ""}>
              <input
                type="radio"
                name="payment"
                checked={customer.payment === "change"}
                onChange={() => setCustomer({ ...customer, payment: "change" })}
              />
              <span>
                <b>SÍ, NECESITO CAMBIO</b>
          ...
          {customer.payment === "change" && (
            <label className="change-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setCustomer({ ...customer, changeFor: event.target.value })
                }
                placeholder="Ej. 200"
              />
              <b>MXN</b>
      ...
      <aside className="delivery-notice">
        <p>{delivery.title}</p>
      ...
      <aside className="privacy-note">
        <strong>TUS DATOS, SOLO PARA TU PEDIDO.</strong>
        <span>
          Se usan para preparar y entregar esta orden; se comparten con El
          Truchita en tu mensaje de WhatsApp.
        </span>
      </aside>
      <aside className="order-confirmation">
        <p>IMPORTANTE</p>
        <strong>
          TU ORDEN SE TRABAJARÁ HASTA QUE SEA CONFIRMADA POR EL NEGOCIO.
        </strong>
        <span>
...
function MenuSection({
  ...
  title: string;
  products: Product[];
  onChoose: (product: Product, options?: CustomizeOptions) => void;
  accent?: boolean;
}) {
  const isCorn = products.some((product) => product.service === "corn");
  return (
    <div
      className={accent ? "menu-section menu-section-accent" : "menu-section"}
    >
      <header>
        <p>{eyebrow}</p>
        <h2>{title}</h2>
        {isCorn && (
          <p className="elote-extra-note">
            Elige tu picante. Si agregas ingredientes o más granos, van en una
            <b>charola de aluminio aparte</b> por $25 c/u.
          </p>
        )}
      </header>
      {products.length ? (
          ...
          {products.map((product, index) => (
            ...
            >
              ...
              <figure className="product-thumb">
                <img
                  src={productImageForDisplay(product)}
                  alt={"Referencia de " + product.name}
                />
              </figure>
                <div className="product-copy">
                  ...
                  <h3>{product.name}</h3>
                  <span className="product-includes">LLEVA</span>
                  <p>{product.description}</p>
                </div>
              <strong>{money(product.price)}</strong>
              <button type="button" onClick={() => onChoose(product)}>
                ARMA TU CHAROLA <span>+</span>
              </button>
            </article>
          ))}
        </div>
        ...
        <div className="empty-section">
          POR AHORA ESTA SECCIÓN ESTÁ DESCANSANDO EN EL ASADOR. VUELVE PRONTO.
        </div>
      )}
    </div>
  );
...
function BagSection({
  ...
  if (!product || !fillings.length)
    return (
      <div className="bag-section-empty">
        POR AHORA ESTA OPCIÓN NO ESTÁ DISPONIBLE. VUELVE PRONTO.
      </div>
    );
  return (
    <div className="bag-section">
      <div className="bag-section-copy">
        <p className="section-kicker light">
          ABRIMOS LA BOTANA. EL RESTO LO ARMAS A TU GUSTO.
        </p>
        <span className="bag-price">BOTANA + ESQUITE A TU ELECCIÓN</span>
        <h2>
          UN GUSTITO
          <br />
          <em>MÁS.</em>
        ...
        <p>
          Elige la botana y el esquite clásico o especial que quieres dentro.
          Verás el total de tu combinación antes de agregar extras.
        </p>
      </div>
      <figure className="bag-main-photo">
        <img
          src="/botanas-en-bolsa.png"
          alt="Bolsas de botana de distintos sabores sobre una mesa con chiles y limón"
        />
        <figcaption>UNA BOLSA · EL ESQUITE QUE TÚ ELIJAS</figcaption>
      </figure>
      <div className="bag-flavor-area">
        <p>¿QUÉ BOTANA SE TE ANTOJA?</p>
        <div className="bag-flavor-stack">
          {snackFlavors.map((flavor, index) => (
            <button
              type="button"
              className={"bag-flavor flavor-" + flavor.color}
              style={{ "--flavor-index": index } as CSSProperties}
              key={flavor.id}
              onClick={() =>
                onChoose(product, {
                  snackFlavor: flavor,
                  bagFilling: fillings[0],
                })
              }
            >
              <span>0{index + 1}</span>
        ...
        <small className="bag-hint">
          ELIGE TU BOTANA Y DESPUÉS DECIDE QUÉ ESQUITE VA DENTRO
        </small>
      </div>
    </div>
  );
...
function MuyMexicanoSection({
  productsBySection,
  bagFillings,
  onChoose,
}: {
  productsBySection: Record<MenuSectionType, Product[]>;
  bagFillings: Product[];
  onChoose: (product: Product, options?: CustomizeOptions) => void;
}) {
  const groups = [
    {
      name: "CLÁSICOS",
      detail: "La base de siempre, directo del carbón.",
      products: productsBySection.traditional,
    },
    {
      name: "ESPECIALES",
      detail: "Sabores de la casa con el fuego al frente.",
      products: productsBySection.specialty,
    },
    {
      name: "ELOTES",
      detail: "Enteros, asados y sin rodeos.",
      products: productsBySection.elotes,
    },
    {
      name: "UN GUSTITO MÁS",
      detail: "Tu botana y tu esquite preferido, a las brasas.",
      products: bagFillings.length ? productsBySection.bolsa : [],
    },
  ].filter((group) => group.products.length);
  return (
    <div className="mexican-section">
      <header>
        <p>MAÍZ, FUEGO Y TRADICIÓN, COMO DEBE SER.</p>
        <h2>MUY MEXICANO</h2>
      </header>
      <div className="mexican-intro">
        <div>
          <p>100% AL CARBÓN</p>
          <strong>EL SABOR DEL MAÍZ CUANDO LO DEJAS HABLAR.</strong>
          <span>
            Estas preparaciones salen del asador sin mantequilla, epazote ni
            especias añadidas. Mayonesa y queso son opcionales y se sirven
            aparte.
          </span>
        </div>
        <div className="mexican-reference-images">
          <img src="/elote-brasa-real.jpeg" alt="Elote asado a las brasas" />
          <img src="/fogon-carbon-real.jpeg" alt="Preparación sobre brasas" />
        </div>
      </div>
      <div className="mexican-family-grid">
        {groups.map((group) => (
            ...
            <div>
              {group.products.map((product) => (
                <button
                  ...
                  onClick={() =>
                    onChoose(product, {
                      preparation: "muy-mexicano",
                      snackFlavor: snackFlavors[0],
                      bagFilling: bagFillings[0],
                    })
                  }
                >
                  <b>{product.name}</b>
